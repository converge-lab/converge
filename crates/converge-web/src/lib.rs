//! converge-web — the product's web app as a library, composed from
//! converge-ui. Screens are driven by a hash router; the sidebar,
//! breadcrumb, and content all react to the current route and the
//! active group.
//!
//! A library so that distributions can compose it (see [`ext`]): the
//! stock binary is `run(Extensions::default())`; an overlay's web
//! crate is the same call with its own screens passed in. The
//! dependency is strictly one-way — this crate never knows what
//! composes it.
...
/// Mount the composed app: the stock screens plus each extension's.
pub fn run(extensions: Extensions) {
    apply_theme(&read_theme());
    mount_to_body(move || {
        provide_context(extensions.clone());
        view! { <App /> }
    });
}
            ...
            >
                {move || {
                    ...
                    match route.get() {
                        ...
                        // An extension's screen; an unclaimed path keeps
                    ...
                    acct_open.get().then(move || {
                        ...
                        view! {
                            ...
                            <div class="cv-acctmenu">
                                ...
                                // The composed extensions' entries — the
                                // stock build has none.
                                {extensions()
                                    .0
                                    .iter()
                                    .map(|e| {
        ...
        <div class="cv-topbar__crumb">
            ...
            {move || {
                ...
                match route.get() {
                    ...
                    // An extension route's crumb is the owning extension's.
                    Route::Ext(path) => extensions()
                        .find(&path)
                        .map(|(e, _)| e.crumb.to_string())
                        .unwrap_or_default(),
                    r => r.crumb(),
                }
            }}
