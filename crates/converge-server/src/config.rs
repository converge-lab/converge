...
pub struct Auth {
    ...
    /// besides the localhost family. Absent → the issuer is derived
    /// per-request from the `Host` header, and `/mcp` answers 403 to
    /// anything but localhost (fine for dev; set it behind a proxy or
    /// under a container name).
    #[serde(default)]
